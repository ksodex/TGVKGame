...
export const SimpleCard = ({
    ...
    const handleFlip = () => {
        if (isCorrect) setFlippedCard(true)
        else {
            setFlippedCard(true)
    ...
    return <button
        ...
        className="perspective-[1000px] aspect-square p-1"
    >
        <div
            className={`relative transform-3d w-full h-full transform transition-transform duration-500 ${flippedCard ? "rotate-y-180" : ""}`}
        >
            <div
            ...
            <div
                className="absolute rounded-md border-2 border-[#430B51] overflow-hidden w-full h-full flex justify-center items-center text-black rotate-y-180 backface-hidden"
                style={{ perspective: "1000px" }}
            >
                <h1 className="text-3xl font-bold">BACK PLACE</h1>
            </div>
        </div>
    </button>
}
